...
class VueLoaderPlugin {
  apply(compiler) {
    ...
    const templateCompilerRule = {
      loader: require.resolve('./loaders/template-loader'),
      resourceQuery: (query) => {
        const parsed = qs.parse(query.slice(1))
        return parsed.vue !== undefined && parsed.type === 'template'
      },
...
function cloneRule(rule) {
  ...
  const res = { ...rule,
    ...
    resourceQuery: (query) => {
      const parsed = qs.parse(query.slice(1))
      if (parsed.vue === undefined) {
        return false
      }
      if (resource && parsed.lang === undefined) {
        return false
      }
...
function cloneRuleForRenderFn(rule) {
  ...
  const res = {
    ...
    resourceQuery: (query) => {
      const parsed = qs.parse(query.slice(1))
      if (parsed.vue === undefined || parsed.type !== 'template') {
        return false
      }
